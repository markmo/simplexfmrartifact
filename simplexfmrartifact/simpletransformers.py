...
class SimpleTransformersModelArtifact(BentoServiceArtifact):
    ...
    def _load_from_directory(self, path, opts=None, metadata=None, update=False):
        if metadata is None:
            self._load_model_opts(path)
        elif update:
            self._load_model_opts(path)
            self._metadata['opts'].update(metadata)
        else:
    ...
    def pack(self, model, metadata=None, update=False):
        if isinstance(model, str):
            if os.path.isdir(model):
                self._load_from_directory(model, metadata, update)
            else:
